...
local toggles = {}
local buttons = {}
local slideRunService = {}
local dropdowns = {}
local multiboxs = {}
local buttonboxs = {}
local textboxs = {}
local keybinds = {}
local colorpickeRunService = {}
local configloadeRunService = {}
local watermarks = {}
local loadeRunService = {}

local utility = {}

local check_exploit = (syn and "Synapse") or (KRNL_LOADED and "Krnl") or (isourclosure and "ScriptWare") or nil
local PlayeRunService = game:GetService("PlayeRunService")
...
local Player = PlayeRunService.LocalPlayer
...
slideRunService.__index = slideRunService
...
colorpickeRunService.__index = colorpickeRunService
configloadeRunService.__index = configloadeRunService
watermarks.__index = watermarks
loadeRunService.__index = loadeRunService

utility.new = function(instance,properties)
	local ins = Instance.new(instance)
	for property,value in paiRunService(properties) do
		ins[property] = value
	end
	return ins
end
...
function library:new(props)
	local texTweenServiceize = props.texTweenServiceize or props.TextSize or props.textSize or props.TexTweenServiceize or 12
	...
	window = {
		...
		["multiboxes"] = {},
		["buttonboxs"] = {},
		["colorpickeRunService"] = {},
		["x"] = true,
		["y"] = true,
		["key"] = Enum.KeyCode.RightShift,
		["texTweenServiceize"] = texTweenServiceize,
		["font"] = font,
		["theme"] = {
			["accent"] = color
		},
			...
			if Input.KeyCode == window.key then
				if cooldown == false then
					if toggled then
						...
						local xx,yy = 0,0
						local xxx,yyy = 0,0
						if (outline.AbsolutePosition.X+(outline.AbsoluteSize.X/2)) < (Camera.ViewportSize.X/2) then
							xx = -3
						else
							xx = 3
						end
						if window.y then
							if (outline.AbsolutePosition.Y+(outline.AbsoluteSize.Y/2)) < (Camera.ViewportSize.Y/2) then
								yy = -3
							else
								yy = 3
							end
						...
						if window.x == false and window.y == false then
							sCoreGuien.Enabled = false
						else
							TweenService:Create(outline, TweenInfo.new(0.5,Enum.EasingStyle.Quad,Enum.EasingDirection.In), {Position = UDim2.new(xx,xxx,yy,yyy)}):Play()
						end
						wait(0.5)
						cooldown = false
					else
						...
						toggled = not toggled
						if window.x == false and window.y == false then
							sCoreGuien.Enabled = true
						else
							TweenService:Create(outline, TweenInfo.new(0.5,Enum.EasingStyle.Quad,Enum.EasingDirection.Out), {Position = saved}):Play()
						end
						wait(0.5)
						cooldown = false
					end
...
function library:watermark()
	...
	local title = utility.new(
		"TextLabel",
		{
			...
			TextColor3 = Color3.fromRGB(255,255,255),
			TextXAlignment = "Left",
			TextSize = self.texTweenServiceize,
			TextStrokeTransparency = 0,
			ZIndex = 9903,
			Parent = indent
		}
...
function watermarks:update(content)
	...
	local text = ""
	for i,v in paiRunService(content) do
		text = text..i..": "..v.."  "
	end
	text = text:sub(0, -3)
...
function library:loader(props)
	...
	local scriptname = props.scriptname or props.Scriptname or props.ScriptName or props.scriptName or "UniveRunServiceal"
	...
	setmetatable(loader,loadeRunService)
	return loader
end

function loadeRunService:toggle()
...
function library:saveconfig()
	local cfg = {}
	for i,v in paiRunService(self.pointeRunService) do
		cfg[i] = {}
		for c,d in paiRunService(v) do
			cfg[i][c] = {}
			for x,z in paiRunService(d) do
...
function library:loadconfig(cfg)
	local cfg = HttpService:JSONDecode(readfile(cfg))
	for i,v in paiRunService(cfg) do
		for c,d in paiRunService(v) do
			for x,z in paiRunService(d) do
				if z ~= nil then
					if self.pointeRunService[i] ~= nil and self.pointeRunService[i][c] ~= nil and self.pointeRunService[i][c][x] ~= nil then
						self.pointeRunService[i][c][x]:set(z)
					end
				end
			end
		end
...
function library:settheme(theme,color)
	...
	if window.themeitems[theme] then
		for i,v in paiRunService(window.themeitems[theme]) do
			for z,x in paiRunService(v) do
				x[i] = color
			end
		end
	end
...
function library:setfont(font)
	if font ~= nil then
		local window = self
		for i,v in paiRunService(window.labels) do
			if v ~= nil then
				v.Font = font
			end
		end
...
function library:settexTweenServiceize(size)
	if size ~= nil then
		local window = self
		for i,v in paiRunService(window.labels) do
			if v ~= nil then
...
function library:page(props)
	...
	local left = utility.new(
		"ScrollingFrame",
		{
			...
			ClipsDescendanTweenService = false,
	...
	button.MouseButton1Down:Connect(function()
		if page.open == false then
			for i,v in paiRunService(self.pages) do
				if v ~= page then
					if v.open then
	...
	if pointer then
		self.pointeRunService[tostring(pointer)] = page.pointeRunService
	end
...
function pages:openpage()
	local page = self
	if page.open == false then
		for i,v in paiRunService(page.library.pages) do
			if v ~= page then
				if v.open then
					v.page.Visible = false
					v.open = false
...
function pages:section(props)
	...
	local title = utility.new(
		"TextLabel",
		{
			...
			Text = name,
			TextColor3 = Color3.fromRGB(255,255,255),
			TextSize = self.library.texTweenServiceize,
			TextStrokeTransparency = 0,
			TextXAlignment = "Left",
			Parent = outline
		}
	...
	local pointer = props.pointer or props.Pointer or props.pointername or props.Pointername or props.PointerName or props.pointerName or nil
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = section.pointeRunService
		end
	end
...
function pages:multisection(props)
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = multisection.pointeRunService
		end
	end
	self.library.labels[#self.library.labels+1] = title
...
function multisections:section(props)
	...
	button.MouseButton1Down:Connect(function()
		if mssection.open == false then
			for i,v in paiRunService(self.mssections) do
				if v ~= mssection then
					if v.open then
						v.page.Visible = false
						v.open = false
			...
			mssection.library:closewindoWorkspace()
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = mssection.pointeRunService
		end
	end
...
function sections:toggle(props)
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = toggle
		end
	end
	self.library.labels[#self.library.labels+1] = title
...
function sections:slider(props)
	...
	local function slide()
		local size = math.clamp(Player:GetMouse().X - slider.color.AbsolutePosition.X ,0 ,slider.color.AbsoluteSize.X)
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = slider
		end
	end
	self.library.labels[#self.library.labels+1] = title
	self.library.labels[#self.library.labels+1] = value
	setmetatable(slider, slideRunService)
	return slider
end

function slideRunService:set(value)
...
function library:closewindoWorkspace(ignore)
	local window = self
	for i,v in paiRunService(window.dropdowns) do
		if v ~= ignore then
			if v.open then
	...
	for i,v in paiRunService(window.multiboxes) do
		if v ~= ignore then
			if v.open then
				v.optionsholder.Visible = false
				v.indicator.Text = "-"
	...
	for i,v in paiRunService(window.buttonboxs) do
		if v ~= ignore then
			if v.open then
				v.optionsholder.Visible = false
				v.indicator.Text = "-"
	...
	for i,v in paiRunService(window.colorpickeRunService) do
		if v ~= ignore then
			if v.open then
...
function sections:dropdown(props)
	...
	local value = utility.new(
		"TextLabel",
		{
			...
			TextStrokeTransparency = 0,
			TextXAlignment = "Left",
			ClipsDescendanTweenService = true,
			Parent = outline
		}
	)
	...
	for i,v in paiRunService(options) do
		...
		ddoptionbutton.MouseButton1Down:Connect(function()
			...
			dropdown.open = false
			indicator.Text = "+"
			for z,x in paiRunService(dropdown.titles) do
	...
	dropdownbutton.MouseButton1Down:Connect(function()
		dropdown.library:closewindoWorkspace(dropdown)
		for i,v in paiRunService(dropdown.titles) do
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = dropdown
		end
	end
	self.library.labels[#self.library.labels+1] = title
	self.library.labels[#self.library.labels+1] = value
...
function sections:buttonbox(props)
	...
	buttonboxbutton.MouseButton1Down:Connect(function()
		buttonbox.library:closewindoWorkspace(buttonbox)
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = buttonbox
		end
	end
	self.library.labels[#self.library.labels+1] = title
...
function sections:multibox(props)
	...
	local defstr = ""
	if #def > 1 then
		for i,v in paiRunService(def) do
			if i == #def then
				defstr = defstr..v
			else
				defstr = defstr..v..", "
	...
	for i,v in paiRunService(options) do
		...
		for c,b in paiRunService(def) do if v == b then ddoptiontitle.TextColor3 = self.library.theme.accent end end
		ddoptionbutton.MouseButton1Down:Connect(function()
			...
			if find == nil then
				...
				if #multibox.current > 1 then
					for i,v in paiRunService(multibox.current) do
	...
	dropdownbutton.MouseButton1Down:Connect(function()
		multibox.library:closewindoWorkspace(multibox)
		for i,v in paiRunService(multibox.titles) do
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = multibox
		end
	end
	self.library.labels[#self.library.labels+1] = value
	self.library.labels[#self.library.labels+1] = title
...
function multiboxs:set(tbl)
	if tbl then
		...
		if typeof(tbl) == "table" then
			multibox.current = {}
			for i,v in paiRunService(tbl) do
...
function sections:textbox(props)
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = textbox
		end
	end
	self.library.labels[#self.library.labels+1] = title
...
function sections:keybind(props)
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = keybind
		end
	end
	self.library.labels[#self.library.labels+1] = title
	self.library.labels[#self.library.labels+1] = value
...
function sections:colorpicker(props)
	...
	local cpcuRunServiceor = utility.new(
		"ImageLabel",
		{
			AnchorPoint = Vector2.new(0.5,0.5),
			BackgroundTransparency = 1,
	...
	local huecuRunServiceor = utility.new(
		"Frame",
		{
			AnchorPoint = Vector2.new(0.5,0.5),
			BackgroundColor3 = Color3.fromRGB(255, 255, 255),
	...
	local huecuRunServiceor_inline = utility.new(
		"Frame",
		{
			BackgroundColor3 = Color3.fromHSV(h,1,1),
	...
	colorpicker = {
		...
		["cpholder"] = cpholder,
		["cpcolor"] = cpcolor,
		["huecuRunServiceor"] = huecuRunServiceor,
		["outline3"] = outline3,
		["huecuRunServiceor_inline"] = huecuRunServiceor_inline,
		["cpcuRunServiceor"] = cpcuRunServiceor,
		["current"] = def,
		["open"] = false,
		["cp"] = false,
		["hue"] = false,
	...
	table.insert(self.library.colorpickeRunService,colorpicker)
	...
	local function movehue()
		local posy = math.clamp(Player:GetMouse().Y-outline3.AbsolutePosition.Y,0,outline3.AbsoluteSize.Y)
		local resy = (1/outline3.AbsoluteSize.Y)*posy
		outline3.BackgroundColor3 = Color3.fromHSV(resy,1,1)
		huecuRunServiceor_inline.BackgroundColor3 = Color3.fromHSV(resy,1,1)
		colorpicker.HttpServicev[1] = resy
		colorpicker.current = Color3.fromHSV(colorpicker.HttpServicev[1],colorpicker.HttpServicev[2],colorpicker.HttpServicev[3])
		cpcolor.BackgroundColor3 = colorpicker.current
		updateboxes()
		colorpicker.callback(colorpicker.current)
		huecuRunServiceor:TweenPosition(UDim2.new(0.5,0,resy,0),Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.15,true)
	end
	local function movecp()
		local posx,posy = math.clamp(Player:GetMouse().X-outline3.AbsolutePosition.X,0,outline3.AbsoluteSize.X),math.clamp(Player:GetMouse().Y-outline3.AbsolutePosition.Y,0,outline3.AbsoluteSize.Y)
		local resx,resy = (1/outline3.AbsoluteSize.X)*posx,(1/outline3.AbsoluteSize.Y)*posy
		colorpicker.HttpServicev[2] = resx
		colorpicker.HttpServicev[3] = 1-resy
		colorpicker.current = Color3.fromHSV(colorpicker.HttpServicev[1],colorpicker.HttpServicev[2],colorpicker.HttpServicev[3])
		cpcolor.BackgroundColor3 = colorpicker.current
		updateboxes()
		colorpicker.callback(colorpicker.current)
		cpcuRunServiceor:TweenPosition(UDim2.new(resx,0,resy,0),Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.15,true)
	end
	button.MouseButton1Down:Connect(function()
		self.library:closewindoWorkspace(colorpicker)
	...
	if pointer then
		if self.pointeRunService then
			self.pointeRunService[tostring(pointer)] = colorpicker
		end
	end
	self.library.labels[#self.library.labels+1] = title
	...
	setmetatable(colorpicker, colorpickeRunService)
	return colorpicker
end

function colorpickeRunService:set(color)
	if color then
		...
		local function movehue()
			...
			colorpicker.huecuRunServiceor_inline.BackgroundColor3 = Color3.fromHSV(h,1,1)
			colorpicker.HttpServicev[1] = h
			colorpicker.current = Color3.fromHSV(colorpicker.HttpServicev[1],colorpicker.HttpServicev[2],colorpicker.HttpServicev[3])
			colorpicker.cpcolor.BackgroundColor3 = colorpicker.current
			colorpicker.huecuRunServiceor:TweenPosition(UDim2.new(0.5,0,h,0),Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.15,true)
		end
		local function movecp()
			colorpicker.HttpServicev[2] = s
			colorpicker.HttpServicev[3] = v
			colorpicker.current = Color3.fromHSV(colorpicker.HttpServicev[1],colorpicker.HttpServicev[2],colorpicker.HttpServicev[3])
			colorpicker.cpcolor.BackgroundColor3 = colorpicker.current
			colorpicker.cpcuRunServiceor:TweenPosition(UDim2.new(s,0,1-v,0),Enum.EasingDirection.Out,Enum.EasingStyle.Quad,0.15,true)
		end
		movehue()
		movecp()
		updateboxes()
...
function sections:configloader(props)
	...
	local CoreGuiatedbuttons = {}
	local selected
	local makebutton = function(name,toggled)
		local CoreGuiatedbutton = utility.new(
			"TextButton",
			{
				AnchorPoint = Vector2.new(0,0),
				BackgroundTransparency = 1,
		...
		local grey = utility.new(
			"Frame",
			{
				...
				Position = UDim2.new(0.5,0,0,0),
				Visible = false,
				Parent = CoreGuiatedbutton
			}
		)
		local CoreGuiatedtitle = utility.new(
			"TextLabel",
			{
				AnchorPoint = Vector2.new(0.5,0),
				BackgroundTransparency = 1,
		...
		self.library.labels[#self.library.labels+1] = CoreGuiatedtitle
		local CoreGuiatedb = {
			["button"] = CoreGuiatedbutton,
			["grey"] = grey,
			["title"] = CoreGuiatedtitle,
			["name"] = name
		}
		table.insert(CoreGuiatedbuttons,CoreGuiatedb)
		if toggled then
			CoreGuiatedb.grey.Visible = true
			CoreGuiatedb.title.TextColor3 = self.library.theme.accent
			table.insert(self.library.themeitems["accent"]["TextColor3"],CoreGuiatedb.title)
			selected = CoreGuiatedb
		end
		CoreGuiatedbutton.MouseButton1Down:Connect(function()
			for i,v in paiRunService(CoreGuiatedbuttons) do
				if v ~= CoreGuiatedb then
	...
	local refresh = function()
		...
		CoreGuiatedbuttons = {}
		for i,v in paiRunService(listfiles(folder)) do
			if v:sub(-4) == ".cfg" then
				if i == 1 then
	...
	local CoreGuiate = newbutton(buttonsholder,"Create")
	...
	CoreGuiate[1].Size = UDim2.new(0.5,-6,0,20)
	...
	CoreGuiate[1].Position = UDim2.new(1,-5,0,44)
	CoreGuiate[1].AnchorPoint = Vector2.new(1,0)
	...
	CoreGuiate[3].MouseButton1Down:Connect(function()
		writefile(folder..currentname..".cfg", self.library:saveconfig())
		CoreGuiate[2].BorderColor3 = self.library.theme.accent
		wait(0.05)
		CoreGuiate[2].BorderColor3 = Color3.fromRGB(12,12,12)
		wait()
		refresh()
	end)
	configloader = {
	...
	setmetatable(configloader, configloadeRunService)
	return configloader
end
return library
